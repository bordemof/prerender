...
server.onRequest = function(req, res) {
    var _this = this;
    console.log("Arriving request",util.getUrl(req));
...
server.createPage = function(req, res) {
    var _this = this;
    console.log("Creating page with phantom");
...
server._sendResponse = function(req, res, options) {
    // var now = new Date();
    // var response_time = Math.abs( ( req.prerender.start.getTime() - now.getTime() ) / 1000 ) );
    console.log(req.prerender.start);
    console.log("Sending response", ' seconds');
    if (req.prerender.documentHTML) {
